...
def getSupply(supply_one):
    ...
    for i in supply_one:
        ans_list.append(int(i) * (int(i) / ans))
...
with open('stableWithWeitProvider_New.csv', 'w', encoding='utf-8-sig', newline='') as f:
    wt = csv.writer(f)
    wt.writerows(swpn)

# with open('supply_ability_a.csv', 'w', encoding='utf-8-sig', newline='') as f:
#     wt = csv.writer(f)
#     wt.writerows(supply_ability_a_list)
#
# with open('supply_ability_b.csv', 'w', encoding='utf-8-sig', newline='') as f:
#     wt = csv.writer(f)
#     wt.writerows(supply_ability_b_list)
#
# with open('supply_ability_c.csv', 'w', encoding='utf-8-sig', newline='') as f:
#     wt = csv.writer(f)
